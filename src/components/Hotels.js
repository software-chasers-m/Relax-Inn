...
export class Hotels extends Component {
  ...
  render() {
    return (
      this.props.hotelsData &&
      <Container className="pt-4 mb-4">
        <Row className="g-4" >
          <h2 style={{ fontWeight: 'bold', textAlign: 'center', color: '#9e7b7c' }}>Most Popular Hotels</h2>
          {this.props.hotelsData.map((item, index) => {
            return (<Col xs={12} md={6} lg={4} >
              <Card key={index} className="card-container" style={{ width: '320px', height: '350px', borderRadius: '40px', textAlign: 'center' }}>
                <div>
                  ...
                  <Card.Body>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', flexDirection: 'column' }}>
                      <Card.Text className='ml-4'>
                        ...
                        <div>Rate: {item.starRating} <BsFillStarFill /></div>
                      </Card.Text>
                      <Button style={{ backgroundColor: '#9e7b7c', color: '#fff', borderRadius: '25px', cursor: 'pointer', border: 'none' }}
                        id={item.id}
                        value={item.price ? item.price.price.current : 'TBD'}
                        onClick={(e) => this.props.handleRoomsData(e)}
                      >Rooms</Button>
                    </div>
                  </Card.Body>
                </div>
